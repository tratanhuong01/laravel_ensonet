...
function postStoryPicture(IDTaiKhoan) {   
    $.ajax({
        method: "POST",
...
function changeTextsPictureMain(color) {
    var x = 0;
    var y = document.getElementById('outer').offsetHeight -
    ...
    color = color ==  '' ? 'white' : color;
    createStoryPictureMain(document.getElementsByClassName("place-type")[0].value,color,x,y/2,w,h);
    document.getElementsByClassName("content-story-text")[0].innerText = document.getElementsByClassName("place-type")[0].value;
}
