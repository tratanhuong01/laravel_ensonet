function processCommentImage(IDBaiDang,IDBinhLuan,event) {
    $.ajax({
        ...
        success : function(response) {
            $('#' + IDBaiDang + IDBinhLuan + 'fileImagess').clone().appendTo
            ('#' + IDBaiDang + IDBinhLuan + 'FormComment');
            $('#' + IDBaiDang + IDBinhLuan + 'CommentImage').html(response.view);
        },
...
function openStickerComment(IDBaiDang,IDBinhLuan,IDTaiKhoan,IDBinhLuanRep,LoaiBinhLuan,event) {
    var y = event.clientY;
    $.ajax({
        method : "GET",
        ...
        success : function(response) {
            if ($('#' + IDBaiDang + IDBinhLuan + 'modalComment').length > 0) {
                if ($('#' + IDBaiDang + IDBinhLuan + 'modalComment').hasClass('hidden')) {
                    if (y < $(window).height()/2) {
                        $('#' + IDBaiDang + IDBinhLuan + 'modalComment').addClass('top-14')
                        $('#' + IDBaiDang + IDBinhLuan + 'modalComment').removeClass('bottom-14')
                    }
                    else {
                        $('#' + IDBaiDang + IDBinhLuan + 'modalComment').addClass('bottom-14')
                        $('#' + IDBaiDang + IDBinhLuan + 'modalComment').removeClass('top-14')
                    }
                    $('#' + IDBaiDang + IDBinhLuan + 'modalComment').html(response)
                    $('#' + IDBaiDang + IDBinhLuan + 'modalComment').removeClass('hidden')
                }
                else {
...
function postStickerRep(IDTaiKhoan, IDBaiDang, IDBinhLuan, IDBinhLuanRep,IDNhanDan) {
    $.ajaxSetup({
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        }
    ...
    formData.append('IDNhanDan',IDNhanDan);
    formData.append('NoiDungBinhLuan', $('#' + IDTaiKhoan + IDBaiDang + IDBinhLuan + "Write").html())
    $.ajax({
        method: "POST",
...
function openModalEditComment(IDBaiDang,IDBinhLuan) {
    if(document.getElementById('editComments'+ IDBinhLuan + IDBaiDang).style.display == 'none') 
        document.getElementById('editComments'+ IDBinhLuan + IDBaiDang).style.display = 'block'
    else 
        document.getElementById('editComments'+ IDBinhLuan + IDBaiDang).style.display = 'none'
