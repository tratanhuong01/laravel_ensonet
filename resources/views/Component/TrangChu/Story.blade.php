...
<div class="w-full flex mt-4">
    ...
    <div onclick="window.location.href = '{{ url($pathSS) }}'" class="w-1/4 md:w-1/5 p-2 pl-0 relative text-center cursor-pointer">
        <div class="w-full">
    ...
    <div onclick="window.location.href = 'viewstory.html'" class="w-1/4 md:w-1/5 p-1.5 relative text-center cursor-pointer">
        <div class="w-full">
            <img class="w-full object-cover" style="height: 182px;border-radius: 10px;" src="/{{ $value->DuongDan }}" alt="">
        </div>
        <div class="w-full absolute text-left pl-1.5 break-all" style="bottom: 15px">
            <a href=""><b class="text-white text-sm">{{ $value->Ho . ' ' . $value->Ten }}</b></a>
        </div>
        <div class="w-full text-left absolute top-5 left-4">
            <img class="w-9 h-9 rounded-full border-4 border-solid border-blue-500" src="/{{ $value->AnhDaiDien }}" alt="">
        </div>
    </div>
    @endforeach
</div>