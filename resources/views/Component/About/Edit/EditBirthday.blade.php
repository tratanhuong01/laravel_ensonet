    ...
    <div class="w-full p-2 flex dark:text-white font-bold">
        <div class="w-32 text-center relative dark:text-white">
            ...
            bg-gray-200 hidden birthDayFull " style="max-height: 200px;">
                ...
                cursor-pointer dark:bg-dark-third">Tháng {{$i}}</div>@endfor
        </div>
    </div>
    <div class="w-24 text-center relative dark:text-white">
        <button onclick="EventClickYearAbout('birthDayFull',1)" id="DayBirths" class="px-4 py-2.5 dark:bg-dark-third dark:text-white bg-gray-300 
                    font-bold rounded-lg">15 &nbsp;&nbsp;<i class="fas fa-caret-down"></i></button>
        <div class="DayBirth z-50 absolute left-2 top-12 w-80 h-60 overflow-y-auto 
            wrapper-content-right shadow-lg border-gray-300 dark:border-dark-main dark:bg-dark-second 
            bg-gray-200 hidden birthDayFull " style="max-height: 200px;">
            @for($i = 1 ; $i <= 31; $i++) <?php $dt = '&nbsp;&nbsp;<i class="fas fa-caret-down"></i>' ?> <div onclick="choose1('DayBirth','{{$i}}','{{$i.$dt}}',
            'DayBirthInput','DayBirths','DayBirth')" class="w-full p-3 dark:text-white font-bold text-left
...
<div class="w-full p-2 flex dark:text-white font-bold">
    <div class="w-32 text-center relative dark:text-white">
        <button onclick="EventClickYearAbout('birthDayFull',2)" class="px-4 py-2.5 dark:bg-dark-third dark:text-white bg-gray-300 
                    font-bold rounded-lg" id="YearBirths">15 &nbsp;&nbsp;<i class="fas fa-caret-down"></i></button>
        <div class="YearBirth z-50 absolute left-2 top-12 w-80 h-60 overflow-y-auto 
            wrapper-content-right shadow-lg border-gray-300 dark:border-dark-main dark:bg-dark-second 
            bg-gray-200 hidden birthDayFull " style="max-height: 200px;">
            @for($i = $year ; $i > $year - 70; $i--)
            <?php $dt = '&nbsp;&nbsp;<i class="fas fa-caret-down"></i>' ?>
            <div onclick="choose1('YearBirth','{{$i}}','{{$i.$dt}}',
            'YearBirthInput','YearBirths','YearBirth')" class="w-full p-3 dark:text-white font-bold text-left
            cursor-pointer dark:bg-dark-third">{{$i}}</div>@endfor
        </div>
    </div>
    <div class="w-24 text-center relative dark:text-white">
