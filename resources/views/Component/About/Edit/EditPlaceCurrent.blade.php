<div class="w-full PlaceLiveCurrent" id="{{ $data->IDNoiOHienTai }}placeCurrentEdit">
    <input oninput="OninputValueInputAbout('placeWork',this,'LiveCurrents')" onclick="EventClickInputAbout('placeWork',this,'LiveCurrents')" name="" id="IDDiaChiLive" class="w-full my-2 p-3 border-2 border-solid border-gray-200 
            ...
            resize-none outline-none rounded-lg" placeholder="Tỉnh / Thành phố hiện tại" value="{{ $data->TenDiaChi }}">
    ...
    <div class="w-full flex relative h-16 pt-3 ">
        ...
        <div id="btnLuuPlaceCurrent" class=" cursor-pointer bg-1877F2 text-white ml-3 rounded-lg p-2 absolute right-0 font-bold">
            Lưu
        </div>
        <div id="btnHuyPlaceCurrent" class=" cursor-pointer bg-gray-200 dark:bg-dark-third dark:text-white  rounded-lg p-2 absolute right-12 font-bold">
            Hủy
        </div>
    </div>
    <!-- formData -->
    <div id="formDataPlaceCurrent">
        <input name="IDDiaChiLive" type="hidden" id="liveCurrentInput" value="{{ $data->IDDiaChi }}">
        <input name="PrivacyInputLiveCurrent" type="hidden" id="PrivacyInputLiveCurrent" value="{{ $data->IDQuyenRiengTu }}">
    </div>
    <!-- formData -->
</div>