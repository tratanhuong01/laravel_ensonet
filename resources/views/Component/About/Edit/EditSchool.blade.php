...
?>
<div class="w-full school" id="{{ $data->IDHocVan }}schoolEdit">
    ...
    resize-none outline-none rounded-lg" placeholder="Trường học" value="{{ $data->TenTruongHoc }}">
    ...
    dark:bg-dark-third dark:border-dark-main shadow-lg dark:text-white  
    resize-none outline-none rounded-lg" placeholder="Loại Trường" value="{{ $data->LoaiTruong }}">
    ...
    <div class="py-2 px-2 w-full dark:text-white font-bold flex">
        <div class="w-10 h-10 p-2 text-center hover:bg-gray-200 
            ...
            <input onchange="OnChangeCheckBoxAboutOffOrOn(this,'yearAboutSchool')" type="checkbox" class="" name="" id="" style="transform: scale(1.8);">
            @else
    ...
    <div class="w-full p-2 flex dark:text-white font-bold yearAboutSchool">
        ...
        <div class="w-24 text-center relative dark:text-white {{ $data->NamKetThuc == NULL ? 'hidden' : '' }}">
            <input type="button" onclick="EventClickYearAbout('YearSchoolInputss',1)" class="px-4 py-2.5 dark:bg-dark-third dark:text-white bg-gray-200 
                    font-bold rounded-lg" value="{{ $data->NamKetThuc == NULL ? 'Năm' : $data->NamKetThuc }}" id="YearEndSchools">
    ...
    <div class="w-full flex relative h-16 pt-3 ">
        ...
        <div id="btnLuuSchool" class=" cursor-pointer bg-1877F2 text-white ml-3 rounded-lg p-2 absolute right-0 font-bold">
            Lưu
        </div>
        <div id="btnHuySchool" class=" cursor-pointer bg-gray-200 dark:bg-dark-third dark:text-white  rounded-lg p-2 absolute right-12 font-bold">
            Hủy
        </div>
    </div>
    <!-- formData -->
    <div id="formDataSchool" class="hidden">
        <input name="IDTruongHoc" type="hidden" id="schoolNameInput" value="{{ $data->IDTruongHoc }}">
        <input name="TypeSchool" type="hidden" id="schoolTypeInput" value="{{ $data->LoaiTruong }}">
        <input name="YearStartSchoolInput" type="hidden" id="YearStartSchoolInputs" value="{{ $data->NamBatDau }}">
        <input name="YearEndSchoolInput" type="hidden" id="YearEndSchoolInputs" value="{{ $data->NamKetThuc }}">
        <input name="PrivacyInputSchool" type="hidden" id="PrivacyInputSchool" value="{{ $data->IDQuyenRiengTu }}">
    </div>
    <!-- formData -->
</div>