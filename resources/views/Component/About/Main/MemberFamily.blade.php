<li class="w-full py-2 flex relative" id="{{ $value->IDThanhVienGiaDinh }}MemberFamily" style="font-size: 16px;">
    <div class="w-10/12 pb-2 flex">
        ...
        <div class="">
            ...
            <p class="text-sm dark:text-gray-300 text-gray-600" style="font-size:12px;">{{ $value->MoiQuanHe }} -
                {{ $value->TinhTrang }}
            </p>
        </div>
    </div>
    <div class="w-2/12 py-2.5">
        <ul class="w-full flex justify-end">
            ...
            <li class="p-2 dark:text-white text-gray-600">
                ...
                @case('CONGKHAI')
                <i onclick="changePrivacyAboutMain('changeMemberFamily','{{ $value->IDThanhVienGiaDinh }}',this)" class="fas fa-globe-europe text-xl cursor-pointer"></i>
                @break
                @case('CHIBANBE')
                <i onclick="changePrivacyAboutMain('changeMemberFamily','{{ $value->IDThanhVienGiaDinh }}',this)" class="fas fa-user-friends text-xl cursor-pointer"></i>
                @break
                @case('RIENGTU')
                <i onclick="changePrivacyAboutMain('changeMemberFamily','{{ $value->IDThanhVienGiaDinh }}',this)" class="fas fa-lock  text-xl cursor-pointer"></i>
                @break
                @endswitch
            </li>
            <li class="p-2  dark:text-white  text-gray-600">
