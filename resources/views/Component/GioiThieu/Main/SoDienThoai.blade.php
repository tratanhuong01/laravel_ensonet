    ...
    <div class="w-2/12">
        <ul class="float-right flex">
            <li class="p-2 dark:text-white text-gray-600">
                ...
                @case('CONGKHAI')
                <i onclick="changePrivacyAboutMain('changeNumberPhone',
                '{{ $value->IDSoDienThoai }}',this)" class="fas fa-globe-europe text-xl cursor-pointer"></i>
                @break
                @case('CHIBANBE')
                <i onclick="changePrivacyAboutMain('changeNumberPhone',
                '{{ $value->IDSoDienThoai }}',this)" class="fas fa-user-friends text-xl cursor-pointer"></i>
                @break
                @case('RIENGTU')
                <i onclick="changePrivacyAboutMain('changeNumberPhone',
                '{{ $value->IDSoDienThoai }}',this)" class="fas fa-lock  text-xl cursor-pointer"></i>
                @break
                @endswitch
            </li>
            <li class="p-2  dark:text-white  text-gray-600">
