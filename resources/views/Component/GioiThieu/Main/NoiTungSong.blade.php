    ...
    <div class="w-2/12 py-2.5">
        <ul class="w-full flex">
            <li class="p-2 dark:text-white text-gray-600">
                ...
                @case('CONGKHAI')
                <i onclick="changePrivacyAboutMain('changePlaceLived',
            '{{ $value->IDNoiTungSong }}',this)" class="fas fa-globe-europe text-xl cursor-pointer"></i>
                @break
                @case('CHIBANBE')
                <i onclick="changePrivacyAboutMain('changePlaceLived',
            '{{ $value->IDNoiTungSong }}',this)" class="fas fa-user-friends text-xl cursor-pointer"></i>
                @break
                @case('RIENGTU')
                <i onclick="changePrivacyAboutMain('changePlaceLived',
            '{{ $value->IDNoiTungSong }}',this)" class="fas fa-lock  text-xl cursor-pointer"></i>
                @break
                @endswitch
            </li>
            <li onclick="editViewAbout()" class="p-2  dark:text-white  text-gray-600">
