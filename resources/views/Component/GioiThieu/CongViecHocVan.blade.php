<div class="w-full">
    <ul class="w-full py-2 px-4">
        <p class="font-bold text-xm py-2 dark:text-white" style="font-family: system-ui;">
            Công Việc</p>
        <p class="font-bold text-xm py-3 text-1877F2">
            <a href=""><i class="fas fa-plus border-2 py-1.5 px-1.5 text-xm border-solid rounded-full" style="border-color: #1877F2;"></i>&nbsp;&nbsp;
                Thêm nơi làm việc</a>
        </p>
        <div class="w-full hidden">
            <div class="w-full relative">
                <input name="" id="" class="w-full my-2 p-3 border-2 border-solid border-gray-200 
                dark:bg-dark-third dark:border-dark-main shadow-lg dark:text-white  resize-none outline-none rounded-lg" placeholder="Tỉnh / thành phố">
                <div class="absolute hidden top-16 z-30 h-80 bg-gray-200 dark:bg-dark-second overflow-y-auto border-gray-300 rounded-lg
                wrapper-content-right border-2 border-solid dark:border-dark-third shadow-lg w-full" style="max-height: 320px;">
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                </div>
            </div>
            <div class="w-full flex pb-1 relative h-16">
                <div class="bg-gray-200  dark:bg-dark-third dark:text-white  rounded-lg p-2 absolute left-0 font-bold">
                    <i class="fas fa-globe-europe"></i>&nbsp;&nbsp;Công khai
                </div>
                <div class="bg-1877F2 text-white rounded-lg p-2 absolute right-0 font-bold">
                    Lưu
                </div>
                <div class="bg-gray-200  dark:bg-dark-third dark:text-white  rounded-lg p-2 absolute right-12 font-bold">
                    Hủy
                </div>
            </div>
        </div>
        <li class="w-full pb-4 flex" style="font-size: 16px;">
            <ul class="w-full">
                <li class="w-full py-2 flex relative" style="font-size: 16px;">
                    <div class="w-10/12 pb-2 flex">
                        <div class="mr-2 pt-0.5">
                            <img src="/img/avatar.jpg" class="w-12 h-12 object-cover 
                            rounded-full" alt="">
                        </div>
                        <div class="">
                            <p class="dark:text-gray-300 text-xm text-gray-600 pb-2">
                                Làm việc tại Trường Cao đẳng Công nghệ Thông tin - Đại học Đà Nẵng
                            </p>
                            <p class="text-sm dark:text-gray-300 text-gray-600" style="font-size:12px;">
                                12 tháng 8, 2019 - Hiện tại
                                ·
                                Đà Nẵng
                            </p>
                        </div>
                    </div>
                    <div class="w-2/12 py-2.5">
                        <ul class="w-full flex">
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-edit text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-trash-alt text-xl cursor-pointer"></i>
                            </li>
                        </ul>
                    </div>
                </li>
                <li class="w-full py-2 flex relative" style="font-size: 16px;">
                    <div class="w-10/12 pb-2 flex">
                        <div class="mr-2 pt-0.5">
                            <img src="/img/avatar.jpg" class="w-12 h-12 object-cover 
                            rounded-full" alt="">
                        </div>
                        <div class="">
                            <p class="dark:text-gray-300 text-xm text-gray-600 pb-2">YouTuber tại YouTube</p>
                            <p class="text-sm dark:text-gray-300 text-gray-600" style="font-size:12px;">
                                Quảng Nam
                            </p>
                        </div>
                    </div>
                    <div class="w-2/12 py-2.5">
                        <ul class="w-full flex">
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-edit text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-trash-alt text-xl cursor-pointer"></i>
                            </li>
                        </ul>
                    </div>
                </li>
            </ul>
        </li>
        <!--  -->
        <p class="font-bold text-xm py-2 dark:text-white" style="font-family: system-ui;">
            Đại học</p>
        <p class="font-bold text-xm py-3 text-1877F2">
            <a href=""><i class="fas fa-plus border-2 py-1.5 px-1.5 text-xm border-solid rounded-full" style="border-color: #1877F2;"></i>&nbsp;&nbsp;
                Thêm trường đại học</a>
        </p>
        <div class="w-full hidden">
            <div class="w-full relative">
                <input name="" id="" class="w-full my-2 p-3 border-2 border-solid border-gray-200 
                dark:bg-dark-third dark:border-dark-main shadow-lg dark:text-white  resize-none outline-none rounded-lg" placeholder="Tỉnh / thành phố">
                <div class="absolute hidden top-16 z-30 h-80 bg-gray-200 dark:bg-dark-second overflow-y-auto border-gray-300 rounded-lg
                wrapper-content-right border-2 border-solid dark:border-dark-third shadow-lg w-full" style="max-height: 320px;">
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                </div>
            </div>
            <div class="w-full flex pb-1 relative h-16">
                <div class="bg-gray-200  dark:bg-dark-third dark:text-white  rounded-lg p-2 absolute left-0 font-bold">
                    <i class="fas fa-globe-europe"></i>&nbsp;&nbsp;Công khai
                </div>
                <div class="bg-1877F2 text-white rounded-lg p-2 absolute right-0 font-bold">
                    Lưu
                </div>
                <div class="bg-gray-200  dark:bg-dark-third dark:text-white  rounded-lg p-2 absolute right-12 font-bold">
                    Hủy
                </div>
            </div>
        </div>
        <li class="w-full pb-4 flex" style="font-size: 16px;">
            <ul class="w-full">
                <li class="w-full py-2 flex relative" style="font-size: 16px;">
                    <div class="w-10/12 pb-2 flex">
                        <div class="mr-2 pt-0.5">
                            <img src="/img/avatar.jpg" class="w-12 h-12 object-cover 
                            rounded-full" alt="">
                        </div>
                        <div class="">
                            <p class="dark:text-gray-300 text-xm text-gray-600 pb-2">
                                Làm việc tại Trường Cao đẳng Công nghệ Thông tin - Đại học Đà Nẵng
                            </p>
                            <p class="text-sm dark:text-gray-300 text-gray-600" style="font-size:12px;">
                                12 tháng 8, 2019 - Hiện tại
                                ·
                                Đà Nẵng
                            </p>
                        </div>
                    </div>
                    <div class="w-2/12 py-2.5">
                        <ul class="w-full flex">
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-edit text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-trash-alt text-xl cursor-pointer"></i>
                            </li>
                        </ul>
                    </div>
                </li>
                <li class="w-full py-2 flex relative" style="font-size: 16px;">
                    <div class="w-10/12 pb-2 flex">
                        <div class="mr-2 pt-0.5">
                            <img src="/img/avatar.jpg" class="w-12 h-12 object-cover 
                            rounded-full" alt="">
                        </div>
                        <div class="">
                            <p class="dark:text-gray-300 text-xm text-gray-600 pb-2">YouTuber tại YouTube</p>
                            <p class="text-sm dark:text-gray-300 text-gray-600" style="font-size:12px;">
                                Quảng Nam
                            </p>
                        </div>
                    </div>
                    <div class="w-2/12 py-2.5">
                        <ul class="w-full flex">
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-edit text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-trash-alt text-xl cursor-pointer"></i>
                            </li>
                        </ul>
                    </div>
                </li>
            </ul>
        </li>
        <!-- a -->
        <p class="font-bold text-xm py-2 dark:text-white" style="font-family: system-ui;">
            Cao đẳng</p>
        <p class="font-bold text-xm py-3 text-1877F2">
            <a href=""><i class="fas fa-plus border-2 py-1.5 px-1.5 text-xm border-solid rounded-full" style="border-color: #1877F2;"></i>&nbsp;&nbsp;
                Thêm trường cao đẳng</a>
        </p>
        <div class="w-full hidden">
            <div class="w-full relative">
                <input name="" id="" class="w-full my-2 p-3 border-2 border-solid border-gray-200 
                dark:bg-dark-third dark:border-dark-main shadow-lg dark:text-white  resize-none outline-none rounded-lg" placeholder="Tỉnh / thành phố">
                <div class="absolute hidden top-16 z-30 h-80 bg-gray-200 dark:bg-dark-second overflow-y-auto border-gray-300 rounded-lg
                wrapper-content-right border-2 border-solid dark:border-dark-third shadow-lg w-full" style="max-height: 320px;">
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                </div>
            </div>
            <div class="w-full flex pb-1 relative h-16">
                <div class="bg-gray-200  dark:bg-dark-third dark:text-white  rounded-lg p-2 absolute left-0 font-bold">
                    <i class="fas fa-globe-europe"></i>&nbsp;&nbsp;Công khai
                </div>
                <div class="bg-1877F2 text-white rounded-lg p-2 absolute right-0 font-bold">
                    Lưu
                </div>
                <div class="bg-gray-200  dark:bg-dark-third dark:text-white  rounded-lg p-2 absolute right-12 font-bold">
                    Hủy
                </div>
            </div>
        </div>
        <li class="w-full pb-4 flex" style="font-size: 16px;">
            <ul class="w-full">
                <li class="w-full py-2 flex relative" style="font-size: 16px;">
                    <div class="w-10/12 pb-2 flex">
                        <div class="mr-2 pt-0.5">
                            <img src="/img/avatar.jpg" class="w-12 h-12 object-cover 
                            rounded-full" alt="">
                        </div>
                        <div class="">
                            <p class="dark:text-gray-300 text-xm text-gray-600 pb-2">
                                Làm việc tại Trường Cao đẳng Công nghệ Thông tin - Đại học Đà Nẵng
                            </p>
                            <p class="text-sm dark:text-gray-300 text-gray-600" style="font-size:12px;">
                                12 tháng 8, 2019 - Hiện tại
                                ·
                                Đà Nẵng
                            </p>
                        </div>
                    </div>
                    <div class="w-2/12 py-2.5">
                        <ul class="w-full flex">
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-edit text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-trash-alt text-xl cursor-pointer"></i>
                            </li>
                        </ul>
                    </div>
                </li>
                <li class="w-full py-2 flex relative" style="font-size: 16px;">
                    <div class="w-10/12 pb-2 flex">
                        <div class="mr-2 pt-0.5">
                            <img src="/img/avatar.jpg" class="w-12 h-12 object-cover 
                            rounded-full" alt="">
                        </div>
                        <div class="">
                            <p class="dark:text-gray-300 text-xm text-gray-600 pb-2">YouTuber tại YouTube</p>
                            <p class="text-sm dark:text-gray-300 text-gray-600" style="font-size:12px;">
                                Quảng Nam
                            </p>
                        </div>
                    </div>
                    <div class="w-2/12 py-2.5">
                        <ul class="w-full flex">
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-edit text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-trash-alt text-xl cursor-pointer"></i>
                            </li>
                        </ul>
                    </div>
                </li>
            </ul>
        </li>
        <!-- a -->
        <p class="font-bold text-xm py-2 dark:text-white" style="font-family: system-ui;">
            Trung Học Phổ Thông</p>
        <p class="font-bold text-xm py-3 text-1877F2">
            <a href=""><i class="fas fa-plus border-2 py-1.5 px-1.5 text-xm border-solid rounded-full" style="border-color: #1877F2;"></i>&nbsp;&nbsp;
                Thêm trường THPT</a>
        </p>
        <div class="w-full hidden">
            <div class="w-full relative">
                <input name="" id="" class="w-full my-2 p-3 border-2 border-solid border-gray-200 
                dark:bg-dark-third dark:border-dark-main shadow-lg dark:text-white  resize-none outline-none rounded-lg" placeholder="Tỉnh / thành phố">
                <div class="absolute hidden top-16 z-30 h-80 bg-gray-200 dark:bg-dark-second overflow-y-auto border-gray-300 rounded-lg
                wrapper-content-right border-2 border-solid dark:border-dark-third shadow-lg w-full" style="max-height: 320px;">
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                </div>
            </div>
            <div class="w-full flex pb-1 relative h-16">
                <div class="bg-gray-200  dark:bg-dark-third dark:text-white  rounded-lg p-2 absolute left-0 font-bold">
                    <i class="fas fa-globe-europe"></i>&nbsp;&nbsp;Công khai
                </div>
                <div class="bg-1877F2 text-white rounded-lg p-2 absolute right-0 font-bold">
                    Lưu
                </div>
                <div class="bg-gray-200  dark:bg-dark-third dark:text-white  rounded-lg p-2 absolute right-12 font-bold">
                    Hủy
                </div>
            </div>
        </div>
        <li class="w-full pb-4 flex" style="font-size: 16px;">
            <ul class="w-full">
                <li class="w-full py-2 flex relative" style="font-size: 16px;">
                    <div class="w-10/12 pb-2 flex">
                        <div class="mr-2 pt-0.5">
                            <img src="/img/avatar.jpg" class="w-12 h-12 object-cover 
                            rounded-full" alt="">
                        </div>
                        <div class="">
                            <p class="dark:text-gray-300 text-xm text-gray-600 pb-2">
                                Làm việc tại Trường Cao đẳng Công nghệ Thông tin - Đại học Đà Nẵng
                            </p>
                            <p class="text-sm dark:text-gray-300 text-gray-600" style="font-size:12px;">
                                12 tháng 8, 2019 - Hiện tại
                                ·
                                Đà Nẵng
                            </p>
                        </div>
                    </div>
                    <div class="w-2/12 py-2.5">
                        <ul class="w-full flex">
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-edit text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-trash-alt text-xl cursor-pointer"></i>
                            </li>
                        </ul>
                    </div>
                </li>
                <li class="w-full py-2 flex relative" style="font-size: 16px;">
                    <div class="w-10/12 pb-2 flex">
                        <div class="mr-2 pt-0.5">
                            <img src="/img/avatar.jpg" class="w-12 h-12 object-cover 
                            rounded-full" alt="">
                        </div>
                        <div class="">
                            <p class="dark:text-gray-300 text-xm text-gray-600 pb-2">YouTuber tại YouTube</p>
                            <p class="text-sm dark:text-gray-300 text-gray-600" style="font-size:12px;">
                                Quảng Nam
                            </p>
                        </div>
                    </div>
                    <div class="w-2/12 py-2.5">
                        <ul class="w-full flex">
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-edit text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-trash-alt text-xl cursor-pointer"></i>
                            </li>
                        </ul>
                    </div>
                </li>
            </ul>
        </li>
        <!-- a -->
        <p class="font-bold text-xm py-2 dark:text-white" style="font-family: system-ui;">
            Trung Học Cơ Sở</p>
        <p class="font-bold text-xm py-3 text-1877F2">
            <a href=""><i class="fas fa-plus border-2 py-1.5 px-1.5 text-xm border-solid rounded-full" style="border-color: #1877F2;"></i>&nbsp;&nbsp;
                Thêm trường THCS</a>
        </p>
        <div class="w-full hidden">
            <div class="w-full relative">
                <input name="" id="" class="w-full my-2 p-3 border-2 border-solid border-gray-200 
                dark:bg-dark-third dark:border-dark-main shadow-lg dark:text-white  resize-none outline-none rounded-lg" placeholder="Tỉnh / thành phố">
                <div class="absolute hidden top-16 z-30 h-80 bg-gray-200 dark:bg-dark-second overflow-y-auto border-gray-300 rounded-lg
                wrapper-content-right border-2 border-solid dark:border-dark-third shadow-lg w-full" style="max-height: 320px;">
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                    <div class="w-full p-2 hover:bg-gray-300 dark:hover:bg-dark-third cursor-pointer 
                    dark:text-white flex rounded-lg flex font-bold">
                        <div class="mr-3">
                            <img src="/img/avatar.jpg" class="w-10 h-10 object-cover rounded-full" alt="">
                        </div>
                        <div class="font-bold dark:text-white py-2">
                            Trà Hưởng
                        </div>
                    </div>
                </div>
            </div>
            <div class="w-full flex pb-1 relative h-16">
                <div class="bg-gray-200  dark:bg-dark-third dark:text-white  rounded-lg p-2 absolute left-0 font-bold">
                    <i class="fas fa-globe-europe"></i>&nbsp;&nbsp;Công khai
                </div>
                <div class="bg-1877F2 text-white rounded-lg p-2 absolute right-0 font-bold">
                    Lưu
                </div>
                <div class="bg-gray-200  dark:bg-dark-third dark:text-white  rounded-lg p-2 absolute right-12 font-bold">
                    Hủy
                </div>
            </div>
        </div>
        <li class="w-full pb-4 flex" style="font-size: 16px;">
            <ul class="w-full">
                <li class="w-full py-2 flex relative" style="font-size: 16px;">
                    <div class="w-10/12 pb-2 flex">
                        <div class="mr-2 pt-0.5">
                            <img src="/img/avatar.jpg" class="w-12 h-12 object-cover 
                            rounded-full" alt="">
                        </div>
                        <div class="">
                            <p class="dark:text-gray-300 text-xm text-gray-600 pb-2">
                                Làm việc tại Trường Cao đẳng Công nghệ Thông tin - Đại học Đà Nẵng
                            </p>
                            <p class="text-sm dark:text-gray-300 text-gray-600" style="font-size:12px;">
                                12 tháng 8, 2019 - Hiện tại
                                ·
                                Đà Nẵng
                            </p>
                        </div>
                    </div>
                    <div class="w-2/12 py-2.5">
                        <ul class="w-full flex">
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-edit text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-trash-alt text-xl cursor-pointer"></i>
                            </li>
                        </ul>
                    </div>
                </li>
                <li class="w-full py-2 flex relative" style="font-size: 16px;">
                    <div class="w-10/12 pb-2 flex">
                        <div class="mr-2 pt-0.5">
                            <img src="/img/avatar.jpg" class="w-12 h-12 object-cover 
                            rounded-full" alt="">
                        </div>
                        <div class="">
                            <p class="dark:text-gray-300 text-xm text-gray-600 pb-2">YouTuber tại YouTube</p>
                            <p class="text-sm dark:text-gray-300 text-gray-600" style="font-size:12px;">
                                Quảng Nam
                            </p>
                        </div>
                    </div>
                    <div class="w-2/12 py-2.5">
                        <ul class="w-full flex">
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-edit text-xl cursor-pointer"></i>
                            </li>
                            <li class="p-2  dark:text-white  text-gray-600">
                                <i class="far fa-trash-alt text-xl cursor-pointer"></i>
                            </li>
                        </ul>
                    </div>
                </li>
            </ul>
        </li>
    </ul>
</div>