...
<div class="w-63% mx-auto py-2">
    ...
    dark:bg-gray-main">
        @if (sizeof($data) <= 0) <p class="font-bold dark:text-white text-center mx-auto py-4">Không có bạn bè để hiển thị</p>
            @else
            @for ($i = 0 ; $i < sizeof($data) ; $i++) <div class="relative flex border-2 border-solid dark:border-dark-second  
        border-gray-200 rounded-lg" style="width: 48.5%;margin: 6px;padding: 2%;">
                <div class="w-1/4">
                    <a href=""><img class="w-24 h-24 rounded-lg object-cover" src="/{{ $data[$i][0]->AnhDaiDien }}" alt=""></a>
                </div>
                <div class="w-2/4 py-4 pl-4">
                    <p style="font-family: Arial, Helvetica, sans-serif"><b>
                            <a style="font-size: 17px;" class="dark:text-white" href="">
                                {{ $data[$i][0]->Ho . ' ' .  $data[$i][0]->Ten }}</a></b></p>
                    <p style="color: #65687B;font-size: 14px;">
                        {{ Functions::getMutualFriend($data[$i][0]->IDTaiKhoan,$user[0]->IDTaiKhoan) }}bạn chung
                    </p>
                </div>
