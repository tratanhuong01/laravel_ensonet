<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Facebook</title>
    <link rel="stylesheet" type="text/css" media="screen" href="css/tailwind_second.css">
    <link rel="stylesheet" href="css/style.css">
    <link href="https://fonts.googleapis.com/css2?family=Lato&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.6.3/css/all.css" />
    <link rel="stylesheet" href="https://unpkg.com/tailwindcss@^2/dist/tailwind.min.css" rel="stylesheet">
    <style>
        * {
            font-family: 'Lato', sans-serif;
        }
    </style>
</head>

<body>
    <div class="w-full relative">
        <div class="w-7/12 mx-auto py-2 flex flex-wrap">
            <div class="w-1/4 border-r-2 px-2 border-solid border-gray-200">
                <p class="font-bold text-xl py-2" style="font-family: system-ui;">Giới thiệu</p>
                <ul class="w-full py-2">
                    <li class="cursor-pointer bg-blue-100 text-1877F2 p-2 rounded-lg text-15px font-bold">
                        Tổng quan
                    </li>
                    <li class="cursor-pointer hover:bg-gray-200 color-word p-2 mt-2 rounded-lg text-15px font-bold">
                        Công việc và học vấn
                    </li>
                    <li class="cursor-pointer hover:bg-gray-200 color-word p-2 mt-2 rounded-lg text-15px font-bold">
                        Nơi từng sống
                    </li>
                    <li class="cursor-pointer hover:bg-gray-200 color-word p-2 mt-2 rounded-lg text-15px font-bold">
                        Thông tin cơ bản và liên hệ
                    </li>
                    <li class="cursor-pointer hover:bg-gray-200 color-word p-2 mt-2 rounded-lg text-15px font-bold">
                        Gia đình và các mối liên hệ
                    </li>
                    <li class="cursor-pointer hover:bg-gray-200 color-word p-2 mt-2 rounded-lg text-15px font-bold">
                        Chi tiết về Hưởng
                    </li>
                    <li class="cursor-pointer hover:bg-gray-200 color-word p-2 mt-2 rounded-lg text-15px font-bold">
                        Sự kiện trong đời
                    </li>
                </ul>
            </div>
            <div class="w-3/4">
                <ul class="w-full py-2 px-4">
                    <li class="w-full py-4 flex relative" style="font-size: 16px;">
                        <div class="w-10/12">
                            <i class="fas fa-briefcase text-gray-600 text-2xl"></i>&nbsp;&nbsp;&nbsp;
                            Làm việc tại <b>FacebookApp </b>
                        </div>
                        <div class="w-2/12">
                            <ul class="w-full flex">
                                <li class="p-2 color-word"><i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                                </li>
                                <li class="p-2 color-word"><i class="far fa-edit text-xl cursor-pointer"></i></li>
                                <li class="p-2 color-word"><i class="far fa-trash-alt text-xl cursor-pointer"></i></li>
                            </ul>
                        </div>
                    </li>
                    <li class="w-full pb-4 flex" style="font-size: 16px;">
                        <div class="w-10/12">
                            <i class="fas fa-graduation-cap text-gray-600 text-2xl"></i>&nbsp;
                            Học tại <b>Trường Cao Đẳng Công Nghệ Thông Tin - Đại Học Đà Nẵng</b>
                        </div>
                        <div class="w-2/12">
                            <ul class="w-full flex">
                                <li class="p-2 color-word"><i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                                </li>
                                <li class="p-2 color-word"><i class="far fa-edit text-xl cursor-pointer"></i></li>
                                <li class="p-2 color-word"><i class="far fa-trash-alt text-xl cursor-pointer"></i></li>
                            </ul>
                        </div>
                    </li>
                    <li class="w-full pb-4 flex" style="font-size: 16px;">
                        <div class="w-10/12">
                            <i class="fas fa-home text-gray-600 text-2xl"></i>
                            &nbsp;&nbsp;&nbsp;Sống tại <b>Đà Nẵng</b>
                        </div>
                        <div class="w-2/12">
                            <ul class="w-full flex">
                                <li class="p-2 color-word"><i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                                </li>
                                <li class="p-2 color-word"><i class="far fa-edit text-xl cursor-pointer"></i></li>
                                <li class="p-2 color-word"><i class="far fa-trash-alt text-xl cursor-pointer"></i></li>
                            </ul>
                        </div>
                    </li>
                    <li class="w-full pb-4 flex" style="font-size: 16px;">
                        <div class="w-10/12">
                            &nbsp;&nbsp;<i class="fas fa-map-marker-alt text-gray-600 text-2xl"></i>
                            &nbsp;&nbsp;&nbsp;&nbsp;Đến từ <b>Quảng Nam</b>
                        </div>
                        <div class="w-2/12">
                            <ul class="w-full flex">
                                <li class="p-2 color-word"><i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                                </li>
                                <li class="p-2 color-word"><i class="far fa-edit text-xl cursor-pointer"></i></li>
                                <li class="p-2 color-word"><i class="far fa-trash-alt text-xl cursor-pointer"></i></li>
                            </ul>
                        </div>
                    </li>
                    <li class="w-full pb-4 flex" style="font-size: 16px;">
                        <div class="w-10/12">
                            &nbsp;<i class="fas fa-heart text-gray-600 text-2xl"></i>
                            &nbsp;&nbsp;&nbsp;Độc Thân
                        </div>
                        <div class="w-2/12">
                            <ul class="w-full flex">
                                <li class="p-2 color-word"><i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                                </li>
                                <li class="p-2 color-word"><i class="far fa-edit text-xl cursor-pointer"></i></li>
                                <li class="p-2 color-word"><i class="far fa-trash-alt text-xl cursor-pointer"></i></li>
                            </ul>
                        </div>
                    </li>
                    <li class="w-full pb-4 flex" style="font-size: 16px;">
                        <div class="w-10/12 font-bold flex">
                            <i class="fas fa-phone text-2xl color-word mr-1" style="transform: rotate(90deg);"></i>
                            <div class="pl-3">
                                <p>0354114665</p>
                                <p class="text-sm color-word">Di động</p>
                            </div>
                        </div>
                        <div class="w-2/12">
                            <ul class="w-full flex">
                                <li class="p-2 color-word"><i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                                </li>
                                <li class="p-2 color-word"><i class="far fa-edit text-xl cursor-pointer"></i></li>
                                <li class="p-2 color-word"><i class="far fa-trash-alt text-xl cursor-pointer"></i></li>
                            </ul>
                        </div>
                    </li>
                </ul>
            </div>
            <div class="w-3/4">
                <ul class="w-full py-2 px-4">
                    <li class="w-full pb-6" style="font-size: 16px;">
                        <p class="font-bold text-xm py-1" style="font-family: system-ui;">2019</p>
                        <p><i class="fas fa-graduation-cap text-gray-600 text-2xl"></i>&nbsp;&nbsp;
                            Sinh viên
                        </p>
                    </li>
                    <li class="w-full pb-6" style="font-size: 16px;">
                        <p class="font-bold text-xm py-1" style="font-family: system-ui;">2016</p>
                        <p><i class="fas fa-graduation-cap text-gray-600 text-2xl"></i>&nbsp;&nbsp;
                            Đã tốt nghiệp tại Trường THCS Nguyễn Duy Hiệu
                        </p>
                    </li>
                    <li class="w-full pb-6" style="font-size: 16px;">
                        <p class="font-bold text-xm py-1" style="font-family: system-ui;">2013</p>
                        <p><i class="fas fa-graduation-cap text-gray-600 text-2xl"></i>&nbsp;&nbsp;
                            Đã tốt nghiệp tại Trường TH Trần Cao Vân
                        </p>
                    </li>
                    <li class="w-full pb-6" style="font-size: 16px;">
                        <p class="font-bold text-xm py-1" style="font-family: system-ui;">2007</p>
                        <p><i class="fas fa-graduation-cap text-gray-600 text-2xl"></i>&nbsp;&nbsp;
                            Đã tốt nghiệp tại Trường Mẫu Giáo Bán Công Bình Đình
                        </p>
                    </li>
                </ul>
            </div>
            <div class="w-3/4">
                <ul class="w-full py-2 px-4">
                    <li class="w-full pb-6 flex" style="font-size: 16px;">
                        <div class="w-1/12 py-3">
                            <i class="fas fa-phone text-2xl" style="transform: rotate(90deg);"></i>
                        </div>
                        <div class="w-3/4">
                            <p class="font-bold text-xm py-1" style="font-family: system-ui;">0354114665</p>
                            <p>Di động</p>
                        </div>
                        <div class="w-2/12">
                            <ul class="w-full flex">
                                <li class="p-2 color-word"><i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                                </li>
                                <li class="p-2 color-word"><i class="far fa-edit text-xl cursor-pointer"></i></li>
                                <li class="p-2 color-word"><i class="far fa-trash-alt text-xl cursor-pointer"></i></li>
                            </ul>
                        </div>
                    </li>
                    <li class="w-full pb-6 flex" style="font-size: 16px;">
                        <div class="w-1/12 py-3">
                            <i class="far fa-envelope text-2xl"></i>
                        </div>
                        <div class="w-3/4">
                            <p class="font-bold text-xm py-1" style="font-family: system-ui;">tratanhuong01@gmail.com
                            </p>
                            <p>Email</p>
                        </div>
                        <div class="w-2/12">
                            <ul class="w-full flex">
                                <li class="p-2 color-word"><i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                                </li>
                                <li class="p-2 color-word"><i class="far fa-edit text-xl cursor-pointer"></i></li>
                                <li class="p-2 color-word"><i class="far fa-trash-alt text-xl cursor-pointer"></i></li>
                            </ul>
                        </div>
                    </li>
                    <li class="w-full pb-6 flex" style="font-size: 16px;">
                        <div class="w-1/12 py-3">
                            <i class="fas fa-transgender-alt text-2xl"></i>
                        </div>
                        <div class="w-3/4">
                            <p class="font-bold text-xm py-1" style="font-family: system-ui;">Nam
                            </p>
                            <p>Giới tính</p>
                        </div>
                        <div class="w-2/12">
                            <ul class="w-full flex">
                                <li class="p-2 color-word"><i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                                </li>
                                <li class="p-2 color-word"><i class="far fa-edit text-xl cursor-pointer"></i></li>
                                <li class="p-2 color-word"><i class="far fa-trash-alt text-xl cursor-pointer"></i></li>
                            </ul>
                        </div>
                    </li>
                    <li class="w-full pb-6 flex flex-wrap" style="font-size: 16px;">
                        <div class="w-1/12 py-3">
                            <i class="fas fa-birthday-cake text-2xl"></i>
                        </div>
                        <div class="w-3/4">
                            <p class="font-bold text-xm py-1" style="font-family: system-ui;">1 tháng 10
                            </p>
                            <p>Ngày sinh</p>
                        </div>
                        <div class="w-2/12">
                            <ul class="w-full flex">
                                <li class="p-2 color-word"><i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                                </li>
                                <li class="p-2 color-word"><i class="far fa-edit text-xl cursor-pointer"></i></li>
                                <li class="p-2 color-word"><i class="far fa-trash-alt text-xl cursor-pointer"></i></li>
                            </ul>
                        </div>
                        <div class="w-1/12 py-3">

                        </div>
                        <div class="w-3/4">
                            <p class="font-bold text-xm py-1" style="font-family: system-ui;">2001
                            </p>
                            <p>Năm sinh</p>
                        </div>
                        <div class="w-2/12">
                            <ul class="w-full flex">
                                <li class="p-2 color-word"><i class="fas fa-globe-europe text-xl cursor-pointer"></i>
                                </li>
                                <li class="p-2 color-word"><i class="far fa-edit text-xl cursor-pointer"></i></li>
                                <li class="p-2 color-word"><i class="far fa-trash-alt text-xl cursor-pointer"></i></li>
                            </ul>
                        </div>
                    </li>
                </ul>
            </div>
            <div class="w-3/4">
                <div class="w-full pt-12">
                    <div class="w-full">
                        <p class="font-bold text-xm py-1" style="font-family: system-ui;">
                            Giới thiệu bản thân</p>
                        <p class="font-bold text-xm py-3 text-1877F2">
                            <a href=""><i class="fas fa-plus border-2 py-1 px-1.5  border-solid rounded-full" style="border-color: #1877F2;"></i>&nbsp;&nbsp;
                                Viết một số điều về chính bạn</a>
                        </p>
                    </div>

                    <div class="w-full">
                        <textarea name="" id="" class="w-full my-2 p-2 border-2 border-solid border-gray-200 
                        resize-none outline-none h-28 rounded-lg
                        " placeholder="Giới thiệu bản thân"></textarea>
                        <div class="w-full flex relative">
                            <div class="bg-gray-200 rounded-lg p-2 absolute left-0 font-bold">
                                <i class="fas fa-globe-europe"></i>&nbsp;&nbsp;Công khai
                            </div>
                            <div class="bg-1877F2 text-white rounded-lg p-2 absolute right-0 font-bold">
                                Lưu
                            </div>
                            <div class="bg-gray-200 rounded-lg p-2 absolute right-12 font-bold">
                                Hủy
                            </div>
                        </div>
                    </div>
                </div>
                <div class="w-full pt-12">
                    <div class="w-full">
                        <p class="font-bold text-xm py-1" style="font-family: system-ui;">
                            Cách phát âm đọc tên</p>
                        <p class="font-bold text-xm py-3 text-1877F2">
                            <a href=""><i class="fas fa-plus border-2 py-1 px-1.5  border-solid rounded-full" style="border-color: #1877F2;"></i>&nbsp;&nbsp;
                                Thêm cách đọc tên</a>
                        </p>
                    </div>
                    <div class="w-full">
                        <textarea name="" id="" class="w-full my-2 p-2 border-2 border-solid border-gray-200 
                        resize-none outline-none h-28 rounded-lg
                        " placeholder="Giới thiệu bản thân"></textarea>
                        <div class="w-full flex relative">
                            <div class="bg-gray-200 rounded-lg p-2 absolute left-0 font-bold">
                                <i class="fas fa-globe-europe"></i>&nbsp;&nbsp;Công khai
                            </div>
                            <div class="bg-1877F2 text-white rounded-lg p-2 absolute right-0 font-bold">
                                Lưu
                            </div>
                            <div class="bg-gray-200 rounded-lg p-2 absolute right-12 font-bold">
                                Hủy
                            </div>
                        </div>
                    </div>
                </div>
                <div class="w-full pt-12">
                    <div class="w-full">
                        <p class="font-bold text-xm py-1" style="font-family: system-ui;">
                            Các tên khác</p>
                        <p class="font-bold text-xm py-3 text-1877F2">
                            <a href=""><i class="fas fa-plus border-2 py-1 px-1.5  border-solid rounded-full" style="border-color: #1877F2;"></i>&nbsp;&nbsp;
                                Thêm biệt danh , tên thường gọi..</a>
                        </p>
                    </div>
                    <div class="w-full">
                        <textarea name="" id="" class="w-full my-2 p-2 border-2 border-solid border-gray-200 
                        resize-none outline-none h-28 rounded-lg
                        " placeholder="Biệt danh , tên thường gọi của bạn"></textarea>
                        <div class="w-full flex relative">
                            <div class="bg-gray-200 rounded-lg p-2 absolute left-0 font-bold">
                                <i class="fas fa-globe-europe"></i>&nbsp;&nbsp;Công khai
                            </div>
                            <div class="bg-1877F2 text-white rounded-lg p-2 absolute right-0 font-bold">
                                Lưu
                            </div>
                            <div class="bg-gray-200 rounded-lg p-2 absolute right-12 font-bold">
                                Hủy
                            </div>
                        </div>
                    </div>
                </div>
                <div class="w-full pt-12">
                    <div class="w-full">
                        <p class="font-bold text-xm py-1" style="font-family: system-ui;">
                            Trích dẫn yêu thích</p>
                        <p class="font-bold text-xm py-3 text-1877F2">
                            <a href=""><i class="fas fa-plus border-2 py-1 px-1.5  border-solid rounded-full" style="border-color: #1877F2;"></i>&nbsp;&nbsp;
                                Thêm câu trích dẫn yêu thích của bạn</a>
                        </p>
                    </div>
                    <div class="w-full">
                        <textarea name="" id="" class="w-full my-2 p-2 border-2 border-solid border-gray-200 
                        resize-none outline-none h-28 rounded-lg" placeholder="Câu trích dẫn yêu thích của bạn"></textarea>
                        <div class="w-full flex relative">
                            <div class="bg-gray-200 rounded-lg p-2 absolute left-0 font-bold">
                                <i class="fas fa-globe-europe"></i>&nbsp;&nbsp;Công khai
                            </div>
                            <div class="bg-1877F2 text-white rounded-lg p-2 absolute right-0 font-bold">
                                Lưu
                            </div>
                            <div class="bg-gray-200 rounded-lg p-2 absolute right-12 font-bold">
                                Hủy
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="w-full relative">
        <div class="w-2/5 mx-auto py-2 border-2 border-solid border-gray-100 rounded-lg">
            <div class="w-full py-2 relative">
                <p class="font-bold text-xl text-center" style="font-family: system-ui;">Chọn đối tượng</p>
                <span onclick="" class="cursor-pointer rounded-full bg-gray-200 py-1.5 px-3 text-lg font-bold
                      absolute top-0 right-4">×</span>
            </div>
            <ul class="w-full p-2">
                <li class="w-full flex p-2 cursor-pointer">
                    <div class="w-2/12">
                        <i class="fas fa-globe-europe text-3xl py-4 px-5 bg-gray-200 rounded-full"></i>
                    </div>
                    <div class="w-9/12">
                        <p class="font-bold text-xl py-1" style="font-family: system-ui;">Công khai</p>
                        <p>Mọi người trên hoặc ngoài facebook</p>
                    </div>
                    <div class="w-1/12 p-6">
                        <input type="radio" class="text-3xl h-4" name="state" style="transform: scale(1.5);">
                    </div>
                </li>
                <li class="w-full flex p-2 cursor-pointer">
                    <div class="w-2/12">
                        <i class="fas fa-user-friends text-3xl py-4 px-4 bg-gray-200 rounded-full"></i>
                    </div>
                    <div class="w-9/12">
                        <p class="font-bold text-xl py-1" style="font-family: system-ui;">Bạn bè</p>
                        <p>Bạn bè của bạn trên facebook</p>
                    </div>
                    <div class="w-1/12 p-6">
                        <input type="radio" class="text-3xl h-4" name="state" style="transform: scale(1.5);">
                    </div>
                </li>
                <li class="w-full flex p-2 cursor-pointer">
                    <div class="w-2/12">
                        <i class="fas fa-lock text-3xl py-4 px-5 bg-gray-200 rounded-full"></i>
                    </div>
                    <div class="w-9/12">
                        <p class="font-bold text-xl py-1" style="font-family: system-ui;">Riêng tư</p>
                        <p>Chỉ mình bạn có thể thấy thông tin này</p>
                    </div>
                    <div class="w-1/12 p-6">
                        <input type="radio" class="text-3xl h-4" name="state" style="transform: scale(1.5);">
                    </div>
                </li>
            </ul>
        </div>
    </div>
</body>

</html>