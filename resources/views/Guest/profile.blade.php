...
<body class="dark:bg-dark-main">
    ...
    <div class="w-full dark:bg-dark-second" id="main">
        ...
        @else
            ...
            <!-- head profile -->
            <div class="hidden dark:bg-dark-second pt-10 w-full md:w-4/5 lg:w-3/4 md:mx-auto xl:w-63%"
                id="firstContentProfile">
                @if ($user[0]->IDTaiKhoan == $users[0]->IDTaiKhoan)
                    <div class="w-full relative">
                        <div class="w-full mx-auto relative">
                            <div class="w-full relative h-60 lg:h-96">
                                <a href="" id="ajaxCover"><img class="w-full h-60 object-cover lg:h-96 rounded-lg"
                                        id="anhBia" src="/{{ $user[0]->AnhBia }}" alt=""></a>
                            </div>
                            <div class="z-0 p-2 bg-gray-100 absolute text-center rounded-lg bottom-3 right-3">
                                <form id="form">
                                    {{ csrf_field() }}
                                    <input onchange="changeBia(event)" name="fileBia" type="file" accept="image"
                                        id="changeB" style="display: none" enctype="multipart/form-data">
                                </form>
                        ...
                        " style="background: rgba(0,0,0,0.5);display:none;">
                            ...
                            <div class="w-full absolute text-center top-20 lg:top-6/10">
                                ...
                                border-4 border-solid border-white object-cover" src="/{{ $user[0]->AnhDaiDien }}"
                                        alt=""></a>
                            ...
                            <div class="text-2xl absolute bg-gray-100 rounded-full"
                                ...
                                <form id="formAvatar" method="POST" action="ProcessViewAvatar"
                                    ...
                                    {{ csrf_field() }}
                                    <input onchange="changeAvatar(event)" name="fileAvatar" type="file" accept="image"
                                        id="changeavt" style="display: none">
                    ...
                    <div class="w-full relative">
                        <div class="mx-auto text-center w-63%" style="margin-top: 68px;">
                            ...
                            <div class="w-full pt-2">
                                <button onclick="editDescribeUser('{{ $user[0]->IDTaiKhoan }}',this)"
                                    style="color: #1876F2;font-size: 18px;" state='true'>Chỉnh sửa</button>
                            </div>
                            <br>
                        </div>
                    </div>
                ...
                @else
                    <form action="action" class="dark:bg-dark-second w-full md:w-4/5 lg:w-3/4 md:mx-auto xl:w-full">
                        <div class="w-full relative">
                            <div class="w-full mx-auto relative">
                                <div class="w-full relative h-60 lg:h-96">
                                    <a href="photo/"><img class="w-full h-60 object-cover  lg:h-96"
                                            style="border-radius: 10px;" src="/{{ $users[0]->AnhBia }}" alt=""></a>
                                </div>
                                <div class="w-full absolute text-center top-20 lg:top-6/10">
                                    <img class="w-44 h-44 rounded-full mx-auto
                                border-4 border-solid border-white object-cover" src="/{{ $users[0]->AnhDaiDien }}"
                                        alt=""></a>
                                    <p class="font-bold text-center text-3xl py-2 dark:text-white"
                                        style="font-family: system-ui;">{{ $users[0]->Ho . ' ' . $users[0]->Ten }}
                                    </p>
                                </div>
                            </div>
                        </div>
        ...
        <div class="h-auto p-3 w-20">
            <div class="text-center cursor-pointer py-2 pl-2 pr-1.5 fixed right-3 bottom-4 " id="chatMinize">
                <div onclick="openCreateChat()" class="cursor-pointer">
                    <i
                        class="far fa-edit text-2xl py-2 px-3 pr-2 rounded-full bg-white dark:bg-dark-second 
                                                                                                                                    dark:text-white"></i>
                </div>
            </div>
        </div>
        ...
        <div class="w-full px-4 flex z-0 md:w-full lg:w-full xl:w-1/2
                                                                                                                        ml-auto fixed -bottom-1 right-20"
            id="placeChat">
        </div>
        <!-- place show chat -->
        @endif
