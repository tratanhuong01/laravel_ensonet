...
@else
    @foreach ($allMess as $key => $value)
        ...
        @endphp
        @if (count($el) == 1)
            @if (DataProcessSix::checkOutOrKichGroup($value[0]->IDNhomTinNhan, Session::get('user')[0]->IDTaiKhoan) == false && DataProcessSix::numberMessageNot($value[0]->IDNhomTinNhan, Session::get('user')[0]->IDTaiKhoan) == false)
                        ...
                        <div class="w-full flex py-1 text-sm flex">
                            <div class="w-4/5 text-sm">
                                @isset($value[count($value) - 1])
                                    @php
                                        $sws = explode('#', DataProcess::getState($value[count($value) - 1]->TinhTrang, Session::get('user')[0]->IDTaiKhoan))[1];
                                    @endphp
                                            ...
                                            @case('1')
                                                <span class="text-gray-500 dark:text-white">
                                                    ...
                                                    @switch(json_decode($value[count($value) - 1]->NoiDung)[0]->LoaiTinNhan)
                                                        @case('0')
                                                            {{ substr(json_decode($value[count($value) - 1]->NoiDung)[0]->NoiDungTinNhan, 0, 20) . '...' }}
                                                        @break
                                                        @case('1')
                                    ...
                                    @else
                                        @switch($sws)
                                            @case('0')
                                                <span
                                                    class="font-bold {{ DataProcessFive::checkMessageIsSeen($value[count($value) - 1]->IDTinNhan, Session::get('user')[0]->IDTaiKhoan) == 1 ? 'text-gray-500 ' : 'text-blue-500 ' }}">
                                            ...
                                            @case('1')
                                                <span
                                                    class="{{ DataProcessFive::checkMessageIsSeen($value[count($value) - 1]->IDTinNhan, Session::get('user')[0]->IDTaiKhoan) == 1 ? 'text-gray-500 ' : 'text-blue-500 ' }}  font-bold">
        ...
        @else
            @php
                $sws = $value[count($value) - 1]->TinhTrang == 0 ? '0' : explode('#', DataProcess::getTrangThaiTinNhan($value[count($value) - 1]->TinhTrang, Session::get('user')[0]->IDTaiKhoan))[1];
            @endphp
            @if (DataProcessSix::checkOutOrKichGroup($value[0]->IDNhomTinNhan, Session::get('user')[0]->IDTaiKhoan) == false && DataProcessSix::numberMessageNot($value[0]->IDNhomTinNhan, Session::get('user')[0]->IDTaiKhoan) == false)
                    ...
                    <div class="w-4/5">
                        <div class="w-full">
                            <span class="dark:text-white float-left font-bold">
                                @if ($value[0]->TenNhomTinNhan == '' || $value[0]->TenNhomTinNhan == null)
                                    @php
                                        $name = '';
                                    @endphp
