...
dark:border-dark-third border-2 border-solid border-gray-300 ml-auto">
            ...
            <ul class="flex justify-end">
                ...
                dark:hover:bg-dark-third">
                    <svg role="presentation" height="26px" width="26px" viewBox="-5 -5 30 30">
                        <path id="callVideo{{ (count($messages)==0?'65676B':$messages[0]->IDNhomTinNhan) }}" fill="#{{ (count($messages)==0?'65676B':$messages[0]->IDMauTinNhan) }}" d="M19.492 4.112a.972.972 0 00-1.01.063l-3.052 2.12a.998.998 0 00-.43.822v5.766a1
                ...
                dark:hover:bg-dark-third">
                    <svg fill="#{{ (count($messages)==0?'65676B':$messages[0]->IDMauTinNhan) }}" height="26px" width="26px" viewBox="-4 -4 24 24">
                        <line id="minize{{ (count($messages)==0?'65676B':$messages[0]->IDNhomTinNhan) }}" stroke="#{{ (count($messages)==0?'65676B':$messages[0]->IDMauTinNhan) }}" stroke-linecap="round" stroke-width="2" x1="2" x2="14" y1="8" y2="8"></line>
                    </svg>
                </li>
                ...
                dark:hover:bg-dark-third">
                    <svg height="26px" width="26px" viewBox="-4 -4 24 24">
                        <line id="close1{{ (count($messages)==0?'65676B':$messages[0]->IDNhomTinNhan) }}" stroke="#{{ (count($messages)==0?'65676B':$messages[0]->IDMauTinNhan) }}" stroke-linecap="round" stroke-width="2" x1="2" x2="14" y1="2" y2="14"></line>
                        <line id="close2{{ (count($messages)==0?'65676B':$messages[0]->IDNhomTinNhan) }}" stroke="#{{ (count($messages)==0?'65676B':$messages[0]->IDMauTinNhan) }}" stroke-linecap="round" stroke-width="2" x1="2" x2="14" y1="14" y2="2"></line>
                    </svg>
                </li>
            </ul>
        </div>
    </div>
    <div id='{{ (count($messages)==0?0:$messages[0]->IDNhomTinNhan).$chater[0]->IDTaiKhoan }}Messenges' class="w-full p-1 wrapper-content-right h-88 overflow-y-auto overflow-x-hidden relative">
        @if (count($messages) == 0)
    ...
    <div class="w-full bg-white dark:bg-dark-second relative z-20 pb-2 pt-4 px-1 flex dark:border-dark-third border-t-2 border-solid border-gray-300">
        ...
        <div class="w-1/12 flex py-1">
            <div class="cursor-pointer fill-65676B ">
                ...
                dark:hover:bg-dark-third  p-1 ">
                    <svg id="addOrCancel{{ (count($messages)==0?'65676B':$messages[0]->IDNhomTinNhan) }}" fill="#{{ (count($messages)==0?0:$messages[0]->IDMauTinNhan) }}" id="addOrCancel" onclick="loadTextBoxType()" class="a8c37x1j ms05siws hr662l2t b7h9ocf4 crt8y2ji tftn3vyl" height="20px" width="20px" viewBox="0 0 24 24">
                        <g fill-rule="evenodd">
        ...
        <ul class="three-exten w-1/3 py-1" style="display: block;">
            <li class="float-left cursor-pointer p-1 fill-65676B hover:bg-gray-200 rounded-full 
                ...
                <svg class="a8c37x1j ms05siws hr662l2t b7h9ocf4" height="20px" width="20px" viewBox="0 -1 17 17">
                    <g fill="gray" fill-rule="evenodd">
                        <path id="picture1{{ (count($messages)==0?'65676B':$messages[0]->IDNhomTinNhan) }}" fill="#{{ (count($messages)==0?'65676B':$messages[0]->IDMauTinNhan) }}" d="M2.882 13.13C3.476 4.743 3.773.48 3.773.348L2.195.516c-.7.1-1.478.647-1.478 1.647l1.092 11.419c0 .5.2.9.4 1.3.4.2.7.4.9.4h.4c-.6-.6-.727-.951-.627-2.151z" fill="gray"></path>
                        <circle cx="8.5" cy="4.5" r="1.5" fill="gray"></circle>
                        <path id="picture2{{ (count($messages)==0?'65676B':$messages[0]->IDNhomTinNhan) }}" fill="#{{ (count($messages)==0?'65676B':$messages[0]->IDMauTinNhan) }}" d="M14 6.2c-.2-.2-.6-.3-.8-.1l-2.8 2.4c-.2.1-.2.4 0 .6l.6.7c.2.2.2.6-.1.8-.1.1-.2.1-.4.1s-.3-.1-.4-.2L8.3 8.3c-.2-.2-.6-.3-.8-.1l-2.6 2-.4 3.1c0 .5.2 1.6.7 1.7l8.8.6c.2 0 .5 0 .7-.2.2-.2.5-.7.6-.9l.6-5.9L14 6.2z" fill="#{{ (count($messages)==0?0:$messages[0]->IDMauTinNhan) }}"></path>
                        <path id="picture3{{ (count($messages)==0?'65676B':$messages[0]->IDNhomTinNhan) }}" fill="#{{ (count($messages)==0?'65676B':$messages[0]->IDMauTinNhan) }}" d="M13.9 15.5l-8.2-.7c-.7-.1-1.3-.8-1.3-1.6l1-11.4C5.5 1 6.2.5 7 .5l8.2.7c.8.1 1.3.8 1.3 1.6l-1 11.4c-.1.8-.8 1.4-1.6 1.3z" stroke-linecap="round" stroke-linejoin="round" stroke="#{{ (count($messages)==0?0:$messages[0]->IDMauTinNhan) }}"></path>
                    </g>
                </svg>
            </li>
            <li class="float-left cursor-pointer p-1 fill-65676B  hover:bg-gray-200 rounded-full 
                dark:hover:bg-dark-third">
                <svg id="sticker{{ (count($messages)==0?0:$messages[0]->IDNhomTinNhan) }}" fill="#{{ (count($messages)==0?'65676B':$messages[0]->IDMauTinNhan) }}" class="a8c37x1j ms05siws hr662l2t b7h9ocf4 crt8y2ji" height="20px" width="20px" viewBox="0 0 17 16" x="0px" y="0px">
                    <g fill-rule="evenodd">
                        <circle cx="5.5" cy="5.5" fill="none" r="1"></circle>
                        <circle cx="11.5" cy="4.5" fill="none" r="1"></circle>
            ...
            <li class="float-left cursor-pointer p-1 fill-65676B hover:bg-gray-200 rounded-full 
                dark:hover:bg-dark-third">
                <svg id="gif{{ (count($messages)==0?'65676B':$messages[0]->IDNhomTinNhan) }}" fill="#{{ (count($messages)==0?'65676B':$messages[0]->IDMauTinNhan) }}" class="a8c37x1j ms05siws hr662l2t b7h9ocf4 crt8y2ji" height="20px" width="20px" viewBox="0 0 16 16" x="0px" y="0px">
                    <path d="M.783 12.705c.4.8 1.017 1.206 1.817 1.606 0 0 1.3.594 2.5.694 1 .1 1.9.1 2.9.1s1.9 0 2.9-.1 1.679-.294 2.479-.694c.8-.4 1.157-.906 1.557-1.706.018 0 .4-1.405.5-2.505.1-1.2.1-3 0-4.3-.1-1.1-.073-1.976-.473-2.676-.4-.8-.863-1.408-1.763-1.808-.6-.3-1.2-.3-2.4-.4-1.8-.1-3.8-.1-5.7 0-1 .1-1.7.1-2.5.5s-1.417 1.1-1.817 1.9c0 0-.4 1.484-.5 2.584-.1 1.2-.1 3 0 4.3.1 1 .2 1.705.5 2.505zm10.498-8.274h2.3c.4 0 .769.196.769.696 0 .5-.247.68-.747.68l-1.793.02.022 1.412 1.252-.02c.4 0 .835.204.835.704s-.442.696-.842.696H11.82l-.045 2.139c0 .4-.194.8-.694.8-.5 0-.7-.3-.7-.8l-.031-5.631c0-.4.43-.696.93-.696zm-3.285.771c0-.5.3-.8.8-.8s.8.3.8.8l-.037 5.579c0 .4-.3.8-.8.8s-.8-.4-.8-.8l.037-5.579zm-3.192-.825c.7 0 1.307.183 1.807.683.3.3.4.7.1 1-.2.4-.7.4-1 .1-.2-.1-.5-.3-.9-.3-1 0-2.011.84-2.011 2.14 0 1.3.795 2.227 1.695 2.227.4 0 .805.073 1.105-.127V8.6c0-.4.3-.8.8-.8s.8.3.8.8v1.8c0 .2.037.071-.063.271-.7.7-1.57.991-2.47.991C2.868 11.662 1.3 10.2 1.3 8s1.704-3.623 3.504-3.623z" fill-rule="nonzero"></path>
                </svg>
            </li>
        </ul>
    ...
    dark:border-dark-third border-gray-300">
        ...
        <ul class="dark:border-dark-third w-full border-b-2 border-gray-200 border-solid p-2">
            <li onclick="openChangeColor('{{ (count($messages)==0?0:$messages[0]->IDNhomTinNhan) }}',
