...
@switch($value[$key][0]->IDLoaiThongBao)
@case('CXP1234567')
<div class="w-full py-2 flex cursor-pointer dark:hover:bg-dark-third">
    <div class=" pt-2 relative pl-2">
        <div class="w-16 h-16 rounded-full relative">
            <img src="/{{ $value[$key][0]->AnhDaiDien }}" class="w-14 h-14 rounded-full object-cover 
    ...
    <div class="w-9/12 dark:text-white pl-3">
        @if (count($value[$key]) <= 1) <b>
            {{ $value[$key][0]->Ho . ' ' . $value[$key][0]->Ten }} </b>
            {{ $value[$key][0]->TenLoaiThongBao . ' ' . $value['noiDung']}}.
            @else
            <b>{{ $value[$key][0]->Ho . ' ' . $value[$key][0]->Ten }} và
                {{ count($value[$key]) - 1 }} người khác </b>
            {{ $value[$key][0]->TenLoaiThongBao . ' ' . $value['noiDung']}}.
            @endif
            <br>
            {{ StringUtil::CheckDateTime($value[$key][0]->ThoiGianThongBao) }}
    </div>
    <div class="w-1/12 relative text-center">
...
<div class="w-full py-2 flex cursor-pointer dark:hover:bg-dark-third">
    ...
    <div class="w-9/12 dark:text-white pl-3">
        @if (count($value[$key]) <= 1) <b>
            {{ $value[$key][0]->Ho . ' ' . $value[$key][0]->Ten }} </b> {{ $value[$key][0]->TenLoaiThongBao }}.
            @else
            <b>{{ $value[$key][0]->Ho . ' ' . $value[$key][0]->Ten }} và {{ count($value[$key]) - 1 }} người khác </b> {{ $value[$key][0]->TenLoaiThongBao }}.
            @endif
            <br>
            {{ StringUtil::CheckDateTime($value[$key][0]->ThoiGianThongBao) }}
    </div>
...
<div class="w-full py-2 flex cursor-pointer dark:hover:bg-dark-third">
    ...
    <div class="w-9/12 dark:text-white pl-3">
        @if (count($value[$key]) <= 1) <b>{{ $value[$key][0]->Ho . ' ' . $value[$key][0]->Ten }} </b> {{ $value[$key][0]->TenLoaiThongBao }}.
            @else
