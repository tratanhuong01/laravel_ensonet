    ...
    <form action="" id="formPost" method="POST" enctype="multipart/form-data">
        ...
        <div class="w-full flex px-0 py-2">
            <div class="w-2/12 pt-1">
                <a href=""><img class="w-14 h-14 rounded-full object-cover mx-auto" src="{{ $users[0]->AnhDaiDien }}" alt=""></a>
            </div>
            <div class="w-11/12">
                <p class="p-1 pt-0 font-bold dark:text-white">{{ $users[0]->Ho . ' ' . $users[0]->Ten }}
        ...
        <div class="w-full mt-2.5 wrapper-content-right overflow-y-auto" style="max-height:365px;">
            ...
            resize-none" name="content" placeholder="{{ $users[0]->Ten }} ơi, Bạn đang nghĩ gì thế?"></textarea>
        ...
        <div class="w-full p-2 border-2 border-solid border-gray-300 mt-4">
            <ul class="w-full flex">
                ...
                <li onclick="viewTagFriends()" class="eOpenModal cursor-pointer "><i style="color: #1771E6;font-size: 25px;padding: 2px 4px;" class="fas fa-users-tag"></i>
                    &nbsp;&nbsp;</li>
        ...
        <div class="w-full text-center my-2.5 mx-0">
            <button onclick="postTimeLine('{{ $users[0]->IDTaiKhoan }}')" class="w-full p-2.5 border-none text-white cursor-pointer" id="button-post" style="background-color: gray;border-radius: 20px;cursor:not-allowed;" type="button"><b>Đăng</b></button>
        </div>
    </form>
</div>