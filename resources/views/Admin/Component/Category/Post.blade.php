...
<div class="w-full flex py-3">
    <div class="w-1/2 flex">
        ...
        <div onclick="openFilterOrSort(0)" class="w-48 p-3 mr-5 font-bold flex bg-white cursor-pointer relative">
            ...
            p-1 font-bold absolute top-full left-0 shadow-lg hidden filterOrSort">
                @foreach ($userPost->Filter as $item)
                <div onclick="onClickFilter('post','{{$item->Name}}','{{ json_encode($item->Data) }}',
                '{{ json_encode($userPost) }}',this)" class="p-2 w-full">
                    {{$item->Name}}
                </div>
                @endforeach
            </div>
    ...
    <div class="w-1/2 flex">
        <div onclick="openFilterOrSort(2)" class="w-48 p-3 mr-5 font-bold flex bg-white cursor-pointer relative">
            ...
            <div class="w-48 bg-white border-2 border-solid border-gray-200 
            p-1 font-bold absolute top-full left-0 shadow-lg hidden filterOrSort">
                @foreach ($userPost->Sort as $item)
                <div onclick="onClickSort('post','{{$item->Name}}','{{ json_encode($item->Data) }}',
                '{{ json_encode($userPost) }}',this)" class="p-2 w-full">
                    {{$item->Name}}
                </div>
                @endforeach
        ...
        <input type="text" oninput="onChangeSearch('post',this)" name="" id="" class="w-2/5 ml-4 p-2.5 rounded-3xl 
