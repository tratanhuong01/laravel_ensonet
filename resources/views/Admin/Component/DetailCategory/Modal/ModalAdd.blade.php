        ...
        <div class="w-full p-2">
            <form action="" method="POST" class="w-11/12 mx-auto" id="form">
                ...
                <div class="w-full py-2 flex">
                    ...
                    @switch($value->Type)
                    @case('Color')
                    <div class="cursor-pointer w-12 h-12 mx-auto cursor-pointer m-1 
                    rounded-full" id="colorMessageChange"></div>
                    <button onclick="onChangeColorMessage()" type="button" class="bg-green-500 p-3 rounded-full text-white">
                        Chọn
                    </button>
                    @break
                    @case('Input')
                    ...
                    @break
                    @case('image')
                    <div class="flex items-start w-9/12 relative">
                        <input onchange="onChangeFileImage(event)" type="file" name="file" id="{{ $value->Data->id }}" accept="{{ $value->Accept }}" class="w-1/2 pr-5 my-5 hidden">
                        <label for="{{ $value->Data->id }}" class="flex items-center py-2 px-5 bg-red-500 
                        font-bold text-white mr-5 rounded-lg my-4 absolute top-1/3 ">
                            <i class='bx bx-upload mt-0.5 text-xl mr-3 
                            flex items-center'></i>
                            <span class="flex items-center text-sm">Chọn File....</span>
                        </label>
                        <img src="" id="imageDemo" class="w-56 h-72 ml-40 bg-gray-500 object-cover">
                    </div>
                    @break
                    @case('audio/mp3')
